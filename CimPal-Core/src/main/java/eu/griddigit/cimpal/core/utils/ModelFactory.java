...
public class ModelFactory {
    ...
    public static Map<String, Model> modelLoadMultipleXMLmapping(
            List<File> files,
            Map<String, RDFDatatype> dataTypeMap,
            String xmlBase,
            Lang rdfSourceFormat,
            boolean treeID) throws IOException {
            ...
            try {
                ...
                for (int i = 0; i < streams.size(); i++) {
                    try (InputStream in = streams.get(i)) {
                        // Create a Graph and StreamRDF for parsing with datatype mapping
                        org.apache.jena.graph.Graph graph = org.apache.jena.sparql.graph.GraphFactory.createDefaultGraph();
                        DataTypeStreamRDF sink = new DataTypeStreamRDF(graph, dataTypeMap);

                        // Use RDFParser with datatype mapping
                        org.apache.jena.riot.RDFParser.create().source(in).lang(format).base(xmlBase).parse(sink);

                        // Create a Model from the parsed Graph
                        graph = sink.getGraph();
